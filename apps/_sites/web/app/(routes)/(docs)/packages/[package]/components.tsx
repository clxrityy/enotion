...
import { adjustHexColorOpacity, ColorPalettes, type DocPackage } from "@enotion/core";
  ...
  return (
    <div
      style={{
        ...
        "--foreground-accent": adjustHexColorOpacity(colors?.foreground || "", 0.5),
      } as CSSProperties}
      className="space-y-6"
    >
      {/* Header */}
      ...
      <div className="block">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pb-8">
          {pkg.modules.map((mod) => (
            <Card key={mod.slug}>
              <a href={`/packages/${pkg.slug}/${mod.slug}`} className="px-4 py-6 hover:shadow-lg transition-shadow w-full block rounded-lg">
